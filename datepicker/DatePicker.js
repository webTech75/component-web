...
export default React.createClass({
    ...
    handleBlur() {
        ...
        if(!this.state.inputValue && this.props.onLoseFocus){
            this.setState({
              inputValue: this.makeInputValueString(new Date(this.state.value))
            })
        }
    },
    ...
    <input ref="hiddenInput" type="hidden" id={this.props.id} name={this.props.name} value={this.state.value || ''} data-formattedvalue={this.state.value ? this.state.inputValue : ''} />
        {this.props.showClearButton && !this.props.customControl && <InputGroup.Addon
            onClick={this.props.disabled ? null : null}
            style={{cursor:(!this.props.disabled) ? 'pointer' : 'not-allowed'}}>
        <div style={{opacity: !this.props.disabled ? 1 : 0.5}}>
        <Glyphicon glyph="calendar" onClick = {()=>{this.props.disabled ? null : this.setState({focused:!this.state.focused})}}/>
        </div>
        </InputGroup.Addon>}
        {this.props.children}
    </InputGroup>;
